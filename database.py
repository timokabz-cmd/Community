...
def init_db():
    ...
    # ---------------------------------------------------------------
    # NSSF & Gold Points: new columns on customers + two new tables.
    # Follows the same safe migration pattern as above.
    # ---------------------------------------------------------------
    existing_cols = [row[1] for row in conn.execute("PRAGMA table_info(customers)").fetchall()]
    if 'nssf_registered' not in existing_cols:
        conn.execute("ALTER TABLE customers ADD COLUMN nssf_registered INTEGER DEFAULT 0")
    if 'nssf_number' not in existing_cols:
        conn.execute("ALTER TABLE customers ADD COLUMN nssf_number TEXT DEFAULT NULL")
    if 'nssf_contribution_rate' not in existing_cols:
        conn.execute("ALTER TABLE customers ADD COLUMN nssf_contribution_rate REAL DEFAULT 5.0")

    # nssf_contributions: one row per deposit event. Tracks the NSSF
    # portion split off each savings transaction. remitted=0 means it
    # has been calculated but not yet sent to NSSF; remitted=1 = done.
    ...
    # gold_points_ledger: every earn event is its own row.
    # A member's balance is always SUM(points) — never stored directly,
    # so the history is always auditable and never gets out of sync.
