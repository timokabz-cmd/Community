...
def get_all_saccos():
    ...
    rows = conn.execute("SELECT * FROM sacco_profile ORDER BY id").fetchall()
    conn.close()
    return rows
...
def get_profile(sacco_id):
    if sacco_id is None:
        return None
    conn = get_db_connection()
    row = conn.execute("SELECT * FROM sacco_profile WHERE id = ?", (sacco_id,)).fetchone()
    conn.close()
    return row
...
def create_sacco(data):
    ...
    cursor = conn.execute(f"INSERT INTO sacco_profile ({columns}) VALUES ({placeholders})", list(data.values()))
...
def update_sacco(sacco_id, data):
    ...
    conn.execute(f"UPDATE sacco_profile SET {set_clause} WHERE id = ?", list(data.values()) + [sacco_id])
    conn.commit()
    conn.close()

def save_profile(sacco_id, data):
    """Upserts a SACCO profile. Creates a new SACCO if sacco_id is None, otherwise updates it."""
    if sacco_id is None:
...
def render():
    ...
    st.caption("Organisation-level identity, Emyooga classification, and governance details — captured once per SACCO, used across PDM/Emyooga/funder reporting later.")
    ...
    if role == 'admin':
        ...
        with st.expander("➕ Create New SACCO"):
            ...
            if st.button("Create SACCO"):
                if new_name:
                    ...
                    st.success(f"Created '{new_name}'. Use the sidebar SACCO selector to switch to it.")
        ...
        default_label = next((lbl for lbl, sid in sacco_map.items() if sid == current_sacco_id), labels[0])
        choice = st.selectbox("Editing profile for:", labels, index=labels.index(default_label))
    ...
    with st.expander("SACCO Identity & Registration", expanded=True):
        ...
        with col1:
            parish = st.text_input("Parish (HQ)", value=p.get('parish') or '')
        with col2:
            sub_county = st.text_input("Sub-county", value=p.get('sub_county') or '')
        with col3:
            district = st.text_input("District", value=p.get('district') or '')
        date_of_formation = st.text_input(
            "Date of formation (YYYY-MM-DD)", value=p.get('date_of_formation') or '',
            help="Stored as text so a partial or approximate date can be entered if the exact day isn't known."
        )
        ursb_registration_number = st.text_input(
            "URSB registration number", value=p.get('ursb_registration_number') or '',
            help="Used for both PDM's cooperative registration number and Emyooga's provisional registration number — they refer to the same underlying ID."
        )
        ...
        with col4:
            bank_name = st.text_input("Bank name", value=p.get('bank_name') or '')
        with col5:
            bank_account_number = st.text_input("Bank account number", value=p.get('bank_account_number') or '')
        ...
        st.caption(f"For reference — the system currently has {live_member_count} customer record(s) on file for this SACCO.")
        number_of_enterprise_groups = st.number_input(
            "Number of enterprise groups under the SACCO", min_value=0, step=1,
    ...
    with st.expander("Emyooga Classification"):
        emyooga_category = st.selectbox(
            ...
            index=EMYOOGA_CATEGORIES.index(p['emyooga_category']) if p.get('emyooga_category') in EMYOOGA_CATEGORIES else 0
        )
        ...
        parish_associations = st.text_area(
            "Parish association name(s) under this SACCO — one per line",
            value=p.get('parish_associations') or ''
        )
    ...
    with st.expander("Governance & Compliance"):
        ...
        with col6:
            date_of_last_agm = st.text_input("Date of last AGM (YYYY-MM-DD)", value=p.get('date_of_last_agm') or '')
        with col7:
            date_of_last_audit = st.text_input("Date of last statutory audit (YYYY-MM-DD)", value=p.get('date_of_last_audit') or '')
        auditor_name = st.text_input("Name of auditor / audit firm", value=p.get('auditor_name') or '')
