...
def deposit(account_id, amount, channel="Cash"):
    conn = get_db_connection()
    ...
    conn.commit()
    conn.close()
    post_double_entry("Cash/Bank", "Savings Payable (Members)", amount, f"Savings deposit — account #{account_id} via {channel}")
    return new_balance

def withdraw(account_id, amount, channel="Cash"):
    conn = get_db_connection()
    account = conn.execute("SELECT * FROM savings_accounts WHERE id = ?", (account_id,)).fetchone()
    ...
    conn.commit()
    conn.close()
    post_double_entry("Savings Payable (Members)", "Cash/Bank", amount, f"Savings withdrawal — account #{account_id} via {channel}")
    ...
    with st.form("txn_form", clear_on_submit=True):
        ...
        if submitted:
            ...
            if amount <= 0:
                ...
                new_balance = deposit(account_id, amount, channel)
                st.success(f"Deposited UGX {amount:,.0f} via {channel}. New balance: UGX {new_balance:,.0f}")
            else:
                new_balance, error = withdraw(account_id, amount, channel)
                if error:
                    st.error(error)
                else:
