...
def get_messages(sacco_id, limit=20):
    ...
    rows = conn.execute(
        """SELECT messages_log.*, customers.name as customer_name FROM messages_log
           ...
           WHERE customers.sacco_id = ? ORDER BY messages_log.id DESC LIMIT ?""",
        (sacco_id, limit)
    ).fetchall()
    conn.close()
    return rows
...
def record_repayment(loan_id, amount, sacco_id, method="Mobile Money", reference=None):
    """Simulates an automated mobile money webhook: applies the payment to the loan and its schedule, posts the ledger entry, and fires an instant client confirmation — with no manual staff handling in between."""
    ...
    post_double_entry("Cash/Bank", "Loans Receivable", amount, f"Repayment for loan #{loan_id}", reference, sacco_id=sacco_id)
...
def get_repayments(sacco_id, limit=20):
    conn = get_db_connection()
    rows = conn.execute(
        """SELECT repayments.*, customers.name as customer_name FROM repayments
           JOIN loans ON repayments.loan_id = loans.id JOIN customers ON loans.customer_id = customers.id
           WHERE loans.sacco_id = ? ORDER BY repayments.id DESC LIMIT ?""",
        (sacco_id, limit)
    ).fetchall()
    conn.close()
    return rows
...
def render():
    ...
    st.caption(
        ...
        "message automatically — no staff handling in between."
    )
        ...
        with st.form("webhook_form", clear_on_submit=True):
            ...
            if submitted:
                if amount <= 0:
                    ...
                    new_balance, message = record_repayment(loan_id, amount, sacco_id, method)
    ...
    repayments = get_repayments(sacco_id, limit=20)
    if repayments:
        st.dataframe(
