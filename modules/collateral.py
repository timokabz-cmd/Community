...
def add_collateral(loan_id, description, estimated_value):
    ...
    cur  = conn.cursor()
    cur.execute(
        "INSERT INTO collateral (loan_id, description, estimated_value, status) VALUES (%s,%s,%s,%s)",
...
def get_collateral(loan_id=None, sacco_id=None):
    ...
    if loan_id:
        ...
        cur.execute("""
            SELECT collateral.*, loans.id AS loan_ref, customers.name AS borrower_name
            FROM collateral
...
def render():
    ...
    if items:
        st.dataframe(
            [{"Loan": f"#{c['loan_ref']}", "Borrower": c['borrower_name'],
