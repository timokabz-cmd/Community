...
def add_customer(name, phone, national_id, sacco_id, member_type='Member', occupation=None, photo=None,
                  ...
                  village=None, parish=None, nssf_registered=0, nssf_number=None, nssf_contribution_rate=5.0):
    ...
    cursor = conn.execute(
        """INSERT INTO customers
           (name, phone, national_id, created_at, member_type, occupation, photo,
            ...
            nssf_registered, nssf_number, nssf_contribution_rate)
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
    ...
    # Award 50 Gold Points immediately if they join already NSSF-registered
...
def render():
    ...
    with st.form("add_customer_form", clear_on_submit=True):
        ...
        nssf_status = st.radio(
            "Is this member registered with NSSF?",
            ["Yes", "No"],
            horizontal=True,
            help="Members who are not yet registered will need to register at nssfug.org before their savings contributions begin."
        )
        ...
        if nssf_status == "Yes":
            nssf_number = st.text_input(
                "NSSF Membership Number",
                placeholder="e.g. 1000123456",
                help="Found on the member's NSSF card or registration confirmation."
            )
            nssf_contribution_rate = st.slider(
                "Monthly NSSF contribution rate (%)",
                min_value=1, max_value=20, value=5,
                help="This % is automatically split from each deposit and tracked as an NSSF contribution. Minimum is 5% (NSSF standard). Member can increase this."
            )
        else:
            st.warning(
                "⚠️ This member is not yet registered with NSSF. "
                "They can still be enrolled today, but their NSSF contributions will only begin once registered. "
                "Registration takes 2 minutes at **[nssfug.org](https://www.nssfug.org)** — "
                "or you can update their NSSF number later from their profile."
            )
        # ─────────────────────────────────────────────────────────────
        ...
        if submitted:
            if name and phone:
                ...
                nssf_registered_flag = 1 if nssf_status == "Yes" else 0
                add_customer(
                    ...
                    nssf_registered=nssf_registered_flag,
                    nssf_number=nssf_number,
                    nssf_contribution_rate=float(nssf_contribution_rate)
                )
                if nssf_registered_flag == 1:
                    st.success(f"✅ '{name}' enrolled successfully. NSSF registered — 50 Gold Points awarded! 🥉")
                else:
    ...
    with col2:
        ...
        # ── NSSF & Gold Points in profile ─────────────────────────────
        st.divider()
        nssf_col, points_col = st.columns(2)
        with nssf_col:
            if customer['nssf_registered']:
                st.success(f"🇺🇬 NSSF Registered")
                st.caption(f"Number: {customer['nssf_number'] or 'Not captured'}")
                st.caption(f"Contribution rate: {customer['nssf_contribution_rate'] or 5.0}% per deposit")
            else:
